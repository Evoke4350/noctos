...
class Exporter {
  ...
  Future<ExportResult> export(ExportFormat format) async {
    ...
    if (format == ExportFormat.json) {
      ...
      buf.writeln('\n[sleep_diary_entries]');
      buf.writeln(_csvHeader(_diaryToMap(diary.isNotEmpty ? diary.first : _emptyDiary())));
      for (final e in diary) {
        buf.writeln(_csvRow(_diaryToMap(e)));
      }

      buf.writeln('\n[user_schedules]');
      buf.writeln(_csvHeader(_scheduleToMap(schedule.isNotEmpty ? schedule.first : _emptySchedule())));
      for (final e in schedule) {
        buf.writeln(_csvRow(_scheduleToMap(e)));
      }

      buf.writeln('\n[cbti_weeks]');
      buf.writeln(_csvHeader(_weekToMap(weeks.isNotEmpty ? weeks.first : _emptyWeek())));
      for (final e in weeks) {
        buf.writeln(_csvRow(_weekToMap(e)));
      }

      buf.writeln('\n[caffeine_logs]');
      buf.writeln(_csvHeader(_caffeineToMap(caffeine.isNotEmpty ? caffeine.first : _emptyCaffeine())));
      for (final e in caffeine) {
        buf.writeln(_csvRow(_caffeineToMap(e)));
      }

      buf.writeln('\n[worry_journal_entries]');
      buf.writeln(_csvHeader(_worryToMap(worry.isNotEmpty ? worry.first : _emptyWorry())));
