...
void main() {
  group('remainingMg', () {
    ...
    test('halves after one half-life (5h default)', () {
      expect(remainingMg(200, const Duration(hours: 5)), closeTo(100, 0.0001));
    });

    test('quarters after two half-lives', () {
      expect(remainingMg(200, const Duration(hours: 10)), closeTo(50, 0.0001));
    });

    test('returns 0 for negative elapsed (future)', () {
      expect(remainingMg(200, const Duration(hours: -1)), 0);
    });
  ...
  group('remainingAtTime', () {
    test('sums multiple doses', () {
      ...
      final doses = [
        ...
        (
          consumedAt: DateTime(2026, 5, 12, 7, 0),
          mg: 200,
        ), // 15h ago = 3 half-lives → 25
      ];
      expect(remainingAtTime(doses, now), closeTo(125, 0.0001));
    });
  ...
  group('cutoffForSingleDose', () {
    ...
    test(
      '200mg, 50mg target, 5h half: cutoff is 2 half-lives before bed = 10h',
      () {
        final bedtime = DateTime(2026, 5, 12, 23, 0);
